...
outFile = path.abspath(path.join(basepath, "..", "csv/output.csv"))
...
with open(outFile, 'w') as csvfilefinal:
    ...
    #wz, ax, wz, vx, Il, IR
    ##exit()
    writer.writerow(['ROS Timestamp', 'IMU Z rotation', 'IMU X acceleration', 'cmd_vel Z rotation', 'cmd_vel X translation', 'left motor current', 'right motor current'])
    while iImu < nImu and iVel < nVel and iStatus < nStatus:
        if iVel == nVel and iStatus == nStatus:
            writer.writerow([imu[iImu][0], imu[iImu][1], imu[iImu][2], None, None, None, None])
            ##print('imu')
            iImu = iImu + 1
        elif iImu == nImu and iStatus == nStatus:
            writer.writerow([vel[iVel][0], None, None, vel[iVel][1], vel[iVel][2], None, None])
            ##print('vel')
            iVel = iVel + 1
        elif iImu == nImu and iVel == nVel:
            writer.writerow([status[iStatus][0], None, None, None, None, status[iStatus][1], status[iStatus][2]])
            ##print('status')
            iStatus = iStatus + 1
